 ...
 * tools/widget-parity-check.mjs in CI (the SAME gate shipped to both repos).
 *
...
(function () {
  ...
  // Contract versions this vendored widget understands. This copy orders them
  // OLDEST -> NEWEST (the WP copy orders newest-first) — by design; the parity
  // gate asserts the SET, not the order. pickContractVersion() below uses a
  // last-match-wins scan so this ordering still yields the highest mutual version.
  ...
  function pickContractVersion(serverVersions) {
    ...
    // CLIENT_CONTRACT_VERSIONS is ordered OLDEST -> NEWEST in this copy, so
    // "last match wins" yields the highest mutually-supported version. Returns
    // null when there is no mutual version (the caller then aborts the mount).
    var best = null;
    for (var ci = 0; ci < CLIENT_CONTRACT_VERSIONS.length; ci++) {
      if (serverVersions.indexOf(CLIENT_CONTRACT_VERSIONS[ci]) !== -1) {
        best = CLIENT_CONTRACT_VERSIONS[ci];
      }
    }
    return best;
  }
